...
datasets = ["WinoGrande"]
dataset_tag = "WinoGrande_reconstruct"
...
config: dict[str, [float, int, str, dict]] = {
    ...
    "generated_file": f"./workspace/datasets/{dataset_tag}/generated/",
    # train setting
    "max_num_gpus": 8,
    "batch_size": 64,
    "num_workers": 8,
...
test_set = Dataset(
    checkpoint_folders=[f"{DATASET_ROOT}/WinoGrande"],
    tokenizer=tokenizer,
    expected_iteration=expected_iteration,
    num_texts=config["num_texts"],
    text_tokenizer=config["text_tokenizer"],
    max_text_length=config["max_text_length"],
    real_length=config["real_length"],
    texts=[json.load(open(f"{COND_ROOT}/WinoGrande_train.json", "r", encoding="utf-8"))],
)
    ...
    for batch_idx, (tokens, cond_id, cond_mask) in enumerate(train_loader):
        ...
        if accelerator.is_main_process:
            ...
            if batch_idx % config["save_every"] == 0:
                ...
                if batch_idx == 200:
                    torch.save(state, os.path.join(config["save_folder"], config["tag"] + "200.pth"))
                if batch_idx == 400:
                    torch.save(state, os.path.join(config["save_folder"], config["tag"] + "400.pth"))
                if batch_idx == 600:
                    torch.save(state, os.path.join(config["save_folder"], config["tag"] + "600.pth"))
                if batch_idx == 800:
