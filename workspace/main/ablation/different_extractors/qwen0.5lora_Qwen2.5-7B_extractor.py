...
extractor = "./models/Qwen2.5-7B-Instruct"
...
config: dict[str, [float, int, str, dict]] = {
    # global setting
    "seed": SEED,
    "model_tag": os.path.basename(__file__)[:-3],
    "need_test": False,
    "use_wandb": True,
    # data setting
    "token_size": (8, 128),
    ...
    "extra_condition_module": AutoModelForCausalLM.from_pretrained(
        extractor, torch_dtype="auto", attn_implementation="flash_attention_2"
    ).model.to(accelerator.device),
    "max_text_length": max_text_length,
    "layer_index": 14,
    "model_config": {
        "features": [
            (num_texts, max_text_length, 3584),
            (50, 300, 2000),
            (128, 128, 1024),
            (256, 32, 512),
            (1024, 10, 256),
        ...
        "condition_dim": (num_texts, max_text_length, 3584),
        "kernel_size": 9,
    },
}
