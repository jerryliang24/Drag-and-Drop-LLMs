...
datasets = ["ARC-e", "BoolQ", "WinoGrande", "PIQA", "HellaSwag"]
dataset_tag = "5train_2test"
...
test_set2 = Dataset(
    checkpoint_folders=[f"{DATASET_ROOT}/OBQA"],
    tokenizer=tokenizer,
    expected_iteration=expected_iteration,
    num_texts=config["num_texts"],
    text_tokenizer=config["text_tokenizer"],
    max_text_length=config["max_text_length"],
    real_length=config["real_length"],
    texts=[json.load(open(f"{COND_ROOT}/OBQA_train.json", "r", encoding="utf-8"))],
)
...
test_iterator2 = iter(test_loader2)
    ...
    for batch_idx, (tokens, cond_id, cond_mask) in enumerate(train_loader):
        ...
        if accelerator.is_main_process:
            ...
            if batch_idx % config["save_every"] == 0:
                ...
                if accelerator.is_main_process:
                    print("\nEvaluating on test set2:")
                generate(iterator=test_iterator2, idx=batch_idx // config["save_every"])
                # print("\nEvaluating on test set:")
